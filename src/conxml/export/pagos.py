...
from conxml.export.listado import _fecha
...
def exportar_pagos(
    ...
    pagos = catalogo.consultar_pagos(cliente=cliente)
        ...
        for doc in catalogo.consultar_doctos(pago["id"]):
            ...
            ws.append(
                [
                    ...
                    factura["ruta"] if factura else None,
                ]
            )
            n = ws.max_row
            ...
            if fill_rep is not None:
                ws.cell(row=n, column=6).fill = fill_rep
            if insoluble > 0:
    ...
    for celda in ws_resumen[1]:
        celda.font = ESTILO_ENCABEZADO
    for (cliente_mon, moneda), datos in sorted(resumen.items()):
        ws_resumen.append(
            [cliente_mon, moneda, datos["pagos"], datos["monto"], datos["pendientes"], datos["insoluto"]]
        )
